...
import { adaptCanonicalImagePromptToProviderNeutral } from './provider-neutral-image-prompt-adapter';
...
describe('adaptCanonicalImagePromptToProviderNeutral', () => {
  it('returns deterministic output for identical input and options', () => {
    ...
    expect(adaptCanonicalImagePromptToProviderNeutral(spec, options)).toEqual(
      adaptCanonicalImagePromptToProviderNeutral(spec, options),
    );
  });

  it('produces structured GENERATE mode output', () => {
    const output = adaptCanonicalImagePromptToProviderNeutral(canonicalSpec(), { mode: 'GENERATE' });
  ...
  it('produces structured EDIT mode output', () => {
    const output = adaptCanonicalImagePromptToProviderNeutral(canonicalSpec(), { mode: 'EDIT' });
  ...
  it('keeps future components out of the positive prompt and in forbidden output', () => {
    ...
    const output = adaptCanonicalImagePromptToProviderNeutral(afterA, { mode: 'GENERATE' });
  ...
  it('adds provider-neutral quality negatives', () => {
    const output = adaptCanonicalImagePromptToProviderNeutral(canonicalSpec(), { mode: 'GENERATE' });
  ...
  it('reports missing canonical data instead of inventing it', () => {
    ...
    const output = adaptCanonicalImagePromptToProviderNeutral(spec, { mode: 'GENERATE' });
  ...
  it('produces FULL output with all optional detail sections', () => {
    const output = adaptCanonicalImagePromptToProviderNeutral(canonicalSpec(), {
      mode: 'GENERATE',
      profile: 'FULL',
    });
  ...
  it('produces deterministic COMPACT output by omitting only optional detail sections', () => {
    const spec = canonicalSpec();
    const full = adaptCanonicalImagePromptToProviderNeutral(spec, { mode: 'GENERATE', profile: 'FULL' });
    const compact = adaptCanonicalImagePromptToProviderNeutral(spec, {
      mode: 'GENERATE',
      profile: 'COMPACT',
    });
  ...
  it('does not mutate the canonical spec', () => {
    ...
    adaptCanonicalImagePromptToProviderNeutral(spec, { mode: 'EDIT', profile: 'COMPACT' });
