...
describe('ProjectAssemblerStage - Blueprint/Config Persistence', () => {
  ...
  it('assembled project has structural equivalence for replay/regeneration', () => {
    ...
    // visualDNA is generated by the assembler from config (not passed through)
    // Verify it has the expected structure and key values from config
    expect(project.visualDNA).toBeDefined();
    expect(project.visualDNA.id).toContain('blueprint-1_visual_');
