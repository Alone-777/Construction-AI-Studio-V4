...
import { migrateWorkspaceToKling30, transformPromptToKling30 } from './migrate-video-provider-to-kling30.mjs';

describe('migrateWorkspaceToKling30', () => {
  it('transforms Firefly and Kling 2.5 prompts to Kling 3.0 15s', () => {
    expect(transformPromptToKling30(
      '[OFFICIAL FIREFLY VIDEO JOB] Create exactly 8 seconds of realistic 16:9 image-to-video construction timelapse. During this 8-second clip, work. The clip duration is exactly 8 seconds.',
    )).toContain('[OFFICIAL KLING 3.0 VIDEO JOB]');
    expect(transformPromptToKling30(
      '[OFFICIAL KLING 2.5 VIDEO JOB] Create exactly 5 seconds of realistic 16:9 image-to-video construction timelapse with Kling 2.5. During this 5-second clip, work.',
    )).toContain('exactly 15 seconds');
  });

  it('backs up and migrates an existing workspace without changing job ids or review state', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'construction-ai-kling30-migration-'));
    ...
    expect(result.durationSecondsPerJob).toBe(15);
    ...
    expect(job.prompt).toContain('15 seconds');
