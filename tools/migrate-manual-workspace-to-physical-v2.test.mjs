...
describe('migrateManualWorkspaceToPhysicalV2', () => {
  it('upgrades unfinished Jobs to native V2 while preserving retry identity and attempts', async () => {
    ...
    expect(job1.prompt).not.toContain('TOOL_ACTION_NOT_EXECUTED');
    expect(job1.prompt).not.toContain('INSUFFICIENT_PHYSICAL_PROGRESS');
    ...
    expect(state1.lastReview.retryPrompt).not.toBe(job1.prompt);
    expect(state1.lastReview.retryPrompt).toContain('TOOL_ACTION_NOT_EXECUTED');
    expect(state1.lastReview.retryPrompt).toContain('INSUFFICIENT_PHYSICAL_PROGRESS');
    expect(Array.from(state1.lastReview.retryPrompt).length).toBeLessThanOrEqual(1800);
    expect(retryPrompt.trim()).toBe(state1.lastReview.retryPrompt);
